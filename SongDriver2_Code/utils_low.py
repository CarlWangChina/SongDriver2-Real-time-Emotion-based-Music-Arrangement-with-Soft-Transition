...
melody_vocab = json.load(open('dataset/enhance_melody_vocab.json', 'r'))
chord_vocab = json.load(open('dataset/enhance_chord_vocab.json', 'r'))
note_vocab = json.load(open('dataset/enhance_notes_vocab.json', 'r'))
...
def getDataFromMusicFile(file_path):
    ...
    _, keys, melody_low, _, chord, _, _ = read_txt(file_path)
    np_keys.extend(keys)
    ...
    start_note = np.concatenate([[[note_vocab['S']]] * len(np_melody_low)], axis=0)
