...
class EmoFeatureExtractor:
    ...
    def __call__(self, inputs, contain_S_E=False, is_logit=True):
        ...
        if is_logit:
            ...
            note_ids = inputs['note'].flatten()
        
        for nid in note_ids:
            note = self.id2note[str(nid.item())]
        ...
        if is_logit:
            ...
            chord_ids = inputs['chord'].flatten()
        
        for cid in chord_ids:
            chord = self.id2chord[str(cid.item())]
        ...
        for i in range(batch_size):
            ...
            datas = {
                'tone': tone[i],
                'note': notes[i * 64 + offset: (i + 1) * 64 + offset],
