...
class EmoModel(nn.Module):
    ...
    def forward(self, datas):
        ...
        chord = datas['chord_in'][:, 1:]
        note = datas['note_in'][:, 1:]
...
class EmoModelTest(nn.Module):
    def __init__(self, d_emb_emo=256, va_dim=16):
        super(EmoModelTest, self).__init__()
        # embedding
        self.chord_color_embed = nn.Linear(256, 32)
        self.rhy_ptn_embed = nn.Linear(256, 32, bias=False)
        self.struct_embed_one = nn.Linear(4, 16, bias=False)
...
Ablation Study
'''
class EmoModelNoChordColor(nn.Module):
    def __init__(self, d_emb_emo=256, va_dim=16):
        super(EmoModelNoChordColor, self).__init__()
        # embedding
        self.rhy_ptn_embed = nn.Linear(256, 32, bias=False)
        self.struct_embed_one = nn.Linear(4, 16, bias=False)
        self.struct_embed_two = nn.Linear(2, 16)
    ...
    def forward(self, datas):
        ...
        contour_one = self.contour_embed_one(music_feat[..., 518:521])
        contour_two = self.contour_embed_two(music_feat[..., 521:524])
        music_feat = torch.cat([rhy_ptn, struct_one, struct_two, contour_one, contour_two], dim=-1)
        embed_music_feat = self.music_feat_out(music_feat)
        # Note & Chord Embedding
        chord = datas['chord_in'][:, 1:]
        note = datas['note_in'][:, 1:]
...
class EmoModelNoRhyPtn(nn.Module):
    def __init__(self, d_emb_emo=256, va_dim=16):
        super(EmoModelNoRhyPtn, self).__init__()
        # embedding
        self.chord_color_embed = nn.Linear(256, 32)
        self.struct_embed_one = nn.Linear(4, 16, bias=False)
        self.struct_embed_two = nn.Linear(2, 16)
    ...
    def forward(self, datas):
        ...
        contour_one = self.contour_embed_one(music_feat[..., 518:521])
        contour_two = self.contour_embed_two(music_feat[..., 521:524])
        music_feat = torch.cat([chord_color, struct_one, struct_two, contour_one, contour_two], dim=-1)
        embed_music_feat = self.music_feat_out(music_feat)
        # Note & Chord Embedding
        chord = datas['chord_in'][:, 1:]
        note = datas['note_in'][:, 1:]
...
class EmoModelNoStructEmbed(nn.Module):
    def __init__(self, d_emb_emo=256, va_dim=16):
        super(EmoModelNoStructEmbed, self).__init__()
        # embedding
        self.chord_color_embed = nn.Linear(256, 32)
        self.rhy_ptn_embed = nn.Linear(256, 32, bias=False)
        self.contour_embed_one = nn.Linear(3, 16)
    ...
    def forward(self, datas):
        ...
        contour_one = self.contour_embed_one(music_feat[..., 518:521])
        contour_two = self.contour_embed_two(music_feat[..., 521:524])
        music_feat = torch.cat([rhy_ptn, chord_color, contour_one, contour_two], dim=-1)
        embed_music_feat = self.music_feat_out(music_feat)
        # Note & Chord Embedding
        chord = datas['chord_in'][:, 1:]
        note = datas['note_in'][:, 1:]
...
class EmoModelNoContour(nn.Module):
    def __init__(self, d_emb_emo=256, va_dim=16):
        super(EmoModelNoContour, self).__init__()
        # embedding
        self.chord_color_embed = nn.Linear(256, 32)
        self.rhy_ptn_embed = nn.Linear(256, 32, bias=False)
        self.struct_embed_one = nn.Linear(4, 16, bias=False)
    ...
    def forward(self, datas):
        ...
        struct_one = self.struct_embed_one(music_feat[..., 512:516])
        struct_two = self.struct_embed_two(music_feat[..., 516:518])
        music_feat = torch.cat([rhy_ptn, chord_color, struct_one, struct_two], dim=-1)
        embed_music_feat = self.music_feat_out(music_feat)
        # Note & Chord Embedding
        chord = datas['chord_in'][:, 1:]
        note = datas['note_in'][:, 1:]
